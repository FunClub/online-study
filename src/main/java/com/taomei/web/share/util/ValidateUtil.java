package com.taomei.web.share.util;
...
public class ValidateUtil {
     ...
     */
    public static void createVerificationCodeImg(HttpServletRequest request, HttpServletResponse response){
        // 1.在内存中创建一张图片
        ...
        String random = drawRandomNum((Graphics2D) graphics, "nl");// 根据客户端传递的createTypeFlag标识生成验证码图片
        // 7.将随机数存在session中
        request.getSession().setAttribute("verificationCode", random);
    ...
    private void setBorder(Graphics graphics) {
        ...
        // 边框区域
        graphics.drawRect(1, 1, IMG_WIDTH - 2, IMG_HEIGHT - 2);
    }

    /**
     * 生成验证码随机线条
     * */
    private static void drawRandomLine(Graphics graphics) {
        // 设置颜色
     ...
     */
    private static String drawRandomNum(Graphics2D graphics2d, String... createTypeFlag) {
        // 设置字体
        ...
        if (createTypeFlag.length > 0 && null != createTypeFlag[0]) {
            if (createTypeFlag[0].equals("ch")) {
                ...
                // 截取字母
                return createRandomChar(graphics2d, baseLetter);
            }
        } else {
            // 默认截取数字和字母的组合
            return createRandomChar(graphics2d, baseNumLetter);
     ...
     * @return 随机字符
     */
    private static String createRandomChar(Graphics2D graphics2d, String baseChar) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            sb.append(ch);
            // 正向角度
            graphics2d.rotate(degree * Math.PI / 180, x, 10);
            graphics2d.drawString(ch, x, 40);
            // 反向角度
            graphics2d.rotate(-degree * Math.PI / 180, x, 10);
            x += 30;
        }
